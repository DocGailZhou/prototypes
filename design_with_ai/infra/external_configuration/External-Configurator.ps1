...
function Test-Deployment {
    ...
    $job = Start-Job -ScriptBlock {
        ...
        az k8s-extension list --cluster-name $ClusterName --resource-group $ResourceGroup --cluster-type connectedClusters -o json 2>$null
    } -ArgumentList $script:ClusterName, $script:ResourceGroup

    $extensions = $null
    if (Wait-Job -Job $job -Timeout 120) {
        $extensions = Receive-Job -Job $job
    } else {
        Write-WarnLog "Extension list timed out - skipping extension verification"
        Stop-Job -Job $job
    }
    Remove-Job -Job $job

    if ($extensions -and $extensions.Trim() -ne "") {
        ...
        Write-WarnLog "Could not retrieve extension list (empty response or command failed)"
        Write-InfoLog "Extensions may still be deploying. Check manually with:"
        Write-InfoLog "  az k8s-extension list --cluster-name $script:ClusterName --resource-group $script:ResourceGroup --cluster-type connectedClusters -o table"
        Write-InfoLog "Or check pods from edge device:"
        Write-InfoLog "  kubectl get pods -n azure-iot-operations"
    }
